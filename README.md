...
Note: the full cache contents are not included in the GitHub repository because they are too large. The cache is generated by the notebooks during execution and may contain many intermediate model files, fold-level outputs, and checkpoint artefacts. When the submitted artefact is inspected locally, existing cache files can be reused if they are present. When cloning the repository from GitHub, missing cache files will be regenerated as required by the relevant notebooks.
